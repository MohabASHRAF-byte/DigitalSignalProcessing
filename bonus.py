...
noise_data = {i: np.random.normal(0, 0.5) * 10000 for i in range(100)}
...
# Plot sine wave
axs[0].plot(
    sine_signal.get_signal_indexs(),
    sine_signal.get_signal_values(),
    label="Sine Wave"
)
...
# Plot noise
axs[1].plot(
    noise_signal.get_signal_indexs(),
    noise_signal.get_signal_values(),
    label="Noise",
    color='orange'
)
...
axs[2].plot(
    combined_signal.get_signal_indexs(),
    combined_signal.get_signal_values(),
    label="Sine + Noise",
    color='green'
)
...
axs[3].plot(
    sine_autocorr.get_signal_indexs(),
    sine_autocorr.get_signal_values(),
    label="Sine Autocorrelation",
    color='blue'
)
axs[3].plot(
    combined_autocorr.get_signal_indexs(),
    combined_autocorr.get_signal_values(),
    label="Noisy Autocorrelation",
    color='red'
)
