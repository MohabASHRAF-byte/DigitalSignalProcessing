...
class TaskCorrelation(ttk.Frame):
    ...
    def setup_gui(self):
        open_signal_button = ttk.Button(self, text="Open File", command=self.open_first_signal)
        ...
        open_second_signal_button = ttk.Button(self, text="Open File", command=self.open_second_signal)
        open_second_signal_button.grid(row=2, column=0, padx=5, pady=10)
        self.open_second_signal_label = ttk.Label(self, text="No file uploaded")
        self.open_second_signal_label.grid(row=2, column=1, padx=5, pady=5)

        generate_button = ttk.Button(self, text="Correlate", command=self.correlate)
        generate_button.grid(row=7, column=1, padx=5, pady=10)

    def open_first_signal(self):
        file_path = filedialog.askopenfilename(title="Select First Signal File", filetypes=[("Text files", "*.txt")])
        self.first_signal = ReadSignal(file_path)
        self.open_signal_label.config(text="File uploaded")

    def open_second_signal(self):
        file_path = filedialog.askopenfilename(title="Select Second Signal File", filetypes=[("Text files", "*.txt")])
