...
class Task2(ttk.Frame):
    ...
    def setup_gui(self):
        ...
        self.plot_type = ttk.Combobox(self,
                                      values=["Discrete", "Continuous", "Both"]
                                      )
    ...
    def generate_signal(self):
        try:
            ...
            # Fine sampling for continuous signal
            t_continuous = np.linspace(0, 4, 1000)
            ...
            if self.wave_type.get() == "Sine Wave":
                continuous_signal = \
                    amplitude * np.sin(2 * np.pi * frequency * t_continuous + theta)
                discrete_signal = \
                    amplitude * np.sin(2 * np.pi * frequency * t_discrete + theta)
            elif self.wave_type.get() == "Cosine Wave":
                continuous_signal = \
                    amplitude * np.cos(2 * np.pi * frequency * t_continuous + theta)
                discrete_signal = \
                    amplitude * np.cos(2 * np.pi * frequency * t_discrete + theta)
            else:
                raise ValueError("Invalid wave type selected")

            self.plot_generated_signal(
                t_continuous, continuous_signal, t_discrete, discrete_signal
            )
    ...
    def plot_generated_signal(
        ...
        if plot_type == "Discrete":
            ...
            ax.stem(
                t_discrete, discrete_signal,
                use_line_collection=True, label="Discrete"
            )
