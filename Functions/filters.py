...
def get_filters(filter_type, fs, stop_band_attenuation,
                f1, f2, transition_band
                ):
    delta_f = transition_band / fs
    [N, window_function] = get_n_and_window(stop_band_attenuation, delta_f)
    end_pos = int((N - 1) / 2)

    if filter_type == "Low pass":
        f1 = (f1 + (transition_band / 2)) / fs
        filter_dict = get_low_high_pass_filter(window_function, end_pos, f1)
        return filter_dict
    elif filter_type == "High pass":
        f1 = (f1 - (transition_band / 2)) / fs
        filter_dict = get_low_high_pass_filter(
            window_function, end_pos, f1, filter_type="high"
        )
        return filter_dict
    elif filter_type == "Band Pass":
        f1 = (f1 - (transition_band / 2)) / fs
        f2 = (f2 + (transition_band / 2)) / fs
        filter_dict = get_pass_stop_filter(window_function, end_pos, f1, f2)
        return filter_dict
    elif filter_type == "Band Stop":
        f1 = (f1 + (transition_band / 2)) / fs
        f2 = (f2 - (transition_band / 2)) / fs
        filter_dict = get_pass_stop_filter(
            window_function, end_pos, f1, f2, filter_type="stop"
        )
        return filter_dict
...
def get_pass_stop_filter(window_function, end_pos, f1, f2, filter_type="pass"):
    ...
    for n in range(0, end_pos + 1):
        ...
        if n == 0:
            if filter_type == "pass":
                values.append(2 * (f2 - f1))
            else:
                values.append(1 - (2 * (f2 - f1)))
        else:
            wc1 = 2 * math.pi * f1 * n
            ...
            if filter_type == "pass":
                ho = second_term - first_term
            else:
                ho = first_term - second_term
